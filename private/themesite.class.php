...
class themesite {
    ...
    public function getNewestChildTheme($themeid){
        ...
        if($old['approved'] != 1){
            $sql = sprintf("SELECT themeid, timestamp FROM themes WHERE name=\"%s\" AND mainlcd=\"%s\" AND approved >= 1 AND emailverification = 1 ORDER BY timestamp DESC", $this->db->quote($old['name']), $this->db->quote($old['mainlcd']));
